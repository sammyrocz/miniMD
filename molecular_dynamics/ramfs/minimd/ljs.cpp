...
int main(int argc, char** argv)
{
  ...
  for(int i = 0; i < argc; i++) {
    ...
    if(strcmp(argv[i],"-aoutp") == 0){
      // dumping path for .txt files 
      if(outputpath == NULL)
        outputpath = new char[128];
...
//#ifdef DEBUG
	...
	//if (me == 0) 
	//	initConnection();
	dump.initDump(comm, integrate.ntimes, dump_frequency, dumpdir, analysiscfg,outputpath);	
