...
void divideCommunicator(MPI_Comm &comm, MPI_Comm &lcomm, int rank)
{
	...
	MPI_Allgather(host_name, MPI_MAX_PROCESSOR_NAME, MPI_CHAR, nhost_name, MPI_MAX_PROCESSOR_NAME, MPI_CHAR, comm);
	colour = rank;
	...
	MPI_Comm_split(comm, colour, rank, &lcomm);
}
...
void Modalysis::readConfig(char *analysiscfg)
{
	...
	divideCommunicator(comm, lcomm,myrank);
...
void Modalysis::initAnalyses() {
	...
	for (i=0; i<anum; i++) {
		...
		if(readpath != NULL){
			strcpy(readfile, readpath);
			strcat(readfile, "/");
		}

		strcat(readfile, afname + i * FILENAMELEN);
			
		retval = MPI_File_open (lcomm, readfile, MPI_MODE_RDONLY, MPI_INFO_NULL, &afh[i]);
