...
void Modalysis::compute_histo(double *arr)
{
...
*/
    ...
    MPI_Allreduce(&MIN, &minval, 1, MPI_DOUBLE, MPI_MIN, ucomm);
    MPI_Allreduce(&MAX, &maxval, 1, MPI_DOUBLE, MPI_MAX, ucomm);
    ...
    for (int i = 0; i < nlocal; i++)
    {
        ...
        for (int i = 0; i < numbins; i++)
        {
            if (value >= bin[i][0] && value <= bin[i][1])
                bin[i][2] += 1;
        }
    }
