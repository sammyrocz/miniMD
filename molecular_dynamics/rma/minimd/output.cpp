            ...
            Thermo &thermo, Integrate &integrate, Timer &timer, int screen_yaml)
{
  ...
  // modified 
    ...
    MPI_Comm ucomm = UniverseComm::getinstance()->comm;
  ...
  int nlostall;
  MPI_Allreduce(&nlost, &nlostall, 1, MPI_INT, MPI_SUM, ucomm);
  ...
  MPI_Allreduce(&time_total, &tmp, 1, MPI_DOUBLE, MPI_SUM, ucomm);
  ...
  MPI_Allreduce(&time_force, &tmp, 1, MPI_DOUBLE, MPI_SUM, ucomm);
  tmp /= nprocs;
  ...
  MPI_Allreduce(&time_neigh, &tmp, 1, MPI_DOUBLE, MPI_SUM, ucomm);
  tmp /= nprocs;
  ...
  MPI_Allreduce(&time_comm, &tmp, 1, MPI_DOUBLE, MPI_SUM, ucomm);
  tmp /= nprocs;
  ...
  MPI_Allreduce(&time_other, &tmp, 1, MPI_DOUBLE, MPI_SUM, ucomm);
  tmp /= nprocs;
  ...
  MPI_Allreduce(&tmp, &total, 1, MPI_DOUBLE, MPI_SUM, ucomm);
           ...
           int nhisto, int* histo)
{
  ...
  int ntotal;
  MPI_Allreduce(&n, &ntotal, 1, MPI_INT, MPI_SUM, ucomm);
  double tmp;
  MPI_Allreduce(&ave, &tmp, 1, MPI_DOUBLE, MPI_SUM, ucomm);
  ave = tmp / ntotal;
  MPI_Allreduce(&min, &tmp, 1, MPI_DOUBLE, MPI_MIN, ucomm);
  min = tmp;
  MPI_Allreduce(&max, &tmp, 1, MPI_DOUBLE, MPI_MAX, ucomm);
  max = tmp;
  ...
  MPI_Allreduce(histo, histotmp, nhisto, MPI_INT, MPI_SUM, ucomm);
