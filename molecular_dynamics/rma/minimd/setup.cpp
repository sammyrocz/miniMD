...
int read_lammps_data(Atom &atom, Comm &comm, Neighbor &neighbor, Integrate &integrate, Thermo &thermo, char* file, int units)
{
  ...
  int me;
  MPI_Comm_rank(ucomm, &me);

  /* check that correct # of atoms were created */

  MMD_int natoms;
  //MPI_Allreduce(&atom.nlocal, &natoms, 1, MPI_INT, MPI_SUM, ucomm);
  MPI_Allreduce(&atom.nlocal, &natoms, 1, MPI_LONG_LONG_INT, MPI_SUM, ucomm);
...
int create_atoms(Atom &atom, int nx, int ny, int nz, double rho)
{
  ...
  int iflagall;
  MPI_Allreduce(&iflag, &iflagall, 1, MPI_INT, MPI_MAX, ucomm);
...
void create_velocity(double t_request, Atom &atom, Thermo &thermo)
{
  ...
  double tmp;
  MPI_Allreduce(&vxtot, &tmp, 1, MPI_DOUBLE, MPI_SUM, ucomm);
  vxtot = tmp / atom.natoms;
  MPI_Allreduce(&vytot, &tmp, 1, MPI_DOUBLE, MPI_SUM, ucomm);
  vytot = tmp / atom.natoms;
  MPI_Allreduce(&vztot, &tmp, 1, MPI_DOUBLE, MPI_SUM, ucomm);
