...
int main(int argc, char **argv)
{
	...
	char *path = NULL; // path to read minimd dumped files
	postprocess = 0;
	...
	for (int i = 0; i < argc; i++)
	{
		...
		{
			if (analysiscfg == NULL)
				analysiscfg = new char[256];
			if (analysiscfg != NULL)
				strcpy(analysiscfg, argv[++i]);
			else
				printf("whats wrong!\n");
			continue;
		}
	...
	modalysis.init(me, nprocs, postprocess, totalAtoms, timesteps,path);
