...
void ForceEAM::read_file(const char* filename)
{
  ...
  MPI_Bcast(&flag, 1, MPI_INT, 0, ucomm);
  ...
  MPI_Bcast(&file->mass, 1, MPI_DOUBLE, 0, ucomm);
  MPI_Bcast(&file->nrho, 1, MPI_INT, 0, ucomm);
  MPI_Bcast(&file->drho, 1, MPI_DOUBLE, 0, ucomm);
  MPI_Bcast(&file->nr, 1, MPI_INT, 0, ucomm);
  MPI_Bcast(&file->dr, 1, MPI_DOUBLE, 0, ucomm);
  MPI_Bcast(&file->cut, 1, MPI_DOUBLE, 0, ucomm);
  mass = file->mass;
  ...
  if(sizeof(MMD_float) == 4)
    MPI_Bcast(file->frho, file->nrho, MPI_FLOAT, 0, ucomm);
  else
    MPI_Bcast(file->frho, file->nrho, MPI_DOUBLE, 0, ucomm);

  if(me == 0) grab(fptr, file->nr, file->zr);

  if(sizeof(MMD_float) == 4)
    MPI_Bcast(file->zr, file->nr, MPI_FLOAT, 0, ucomm);
  else
    MPI_Bcast(file->zr, file->nr, MPI_DOUBLE, 0, ucomm);

  if(me == 0) grab(fptr, file->nr, file->rhor);

  if(sizeof(MMD_float) == 4)
    MPI_Bcast(file->rhor, file->nr, MPI_FLOAT, 0, ucomm);
  else
    MPI_Bcast(file->rhor, file->nr, MPI_DOUBLE, 0, ucomm);
