...
MMD_float Thermo::energy(Atom &atom, Neighbor &neighbor, Force* force)
{
  ...
  if(sizeof(MMD_float) == 4)
    MPI_Allreduce(&e_act, &eng, 1, MPI_FLOAT, MPI_SUM, ucomm);
  else
    MPI_Allreduce(&e_act, &eng, 1, MPI_DOUBLE, MPI_SUM, ucomm);
...
MMD_float Thermo::temperature(Atom &atom)
{
  ...
  {
    if(sizeof(MMD_float) == 4)
      MPI_Allreduce(&t_act, &t1, 1, MPI_FLOAT, MPI_SUM, ucomm);
    else
      MPI_Allreduce(&t_act, &t1, 1, MPI_DOUBLE, MPI_SUM, ucomm);
...
MMD_float Thermo::pressure(MMD_float t, Force* force)
{
  ...
  if(sizeof(MMD_float) == 4)
    MPI_Allreduce(&p_act, &virial, 1, MPI_FLOAT, MPI_SUM, ucomm);
  else
    MPI_Allreduce(&p_act, &virial, 1, MPI_DOUBLE, MPI_SUM, ucomm);
